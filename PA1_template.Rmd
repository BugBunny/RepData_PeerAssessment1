...
The mean and median number of steps in the imputed data are `r sprintf("%8.1f",m)` and `r sprintf("%7.0f",med)` respectively.
...
The time at which most steps were taken is `r busy.bee`.
...
```
The total number missing values for steps is `r t.miss`.  
...
The mean and median number of steps in the imputed data are `r sprintf("%8.1f",m)` and `r sprintf("%8.1f",med)` respectively.
